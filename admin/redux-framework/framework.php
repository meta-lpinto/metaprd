    ...
    require_once trailingslashit( get_template_directory() ).'admin/redux-framework/inc/themecheck/class.redux_themecheck.php';
		...
		class ReduxFramework {
			...
			public static function init() {
				...
				if ( strpos( Redux_Helpers::cleanFilePath( trailingslashit( get_template_directory() ).'admin/redux-framework/' ), Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( WP_CONTENT_DIR . '/themes/' ) ) !== false ) {
					...
					if ( strpos( self::$_dir, ABSPATH ) === false ) {
						...
						$is_plugin = false;
						/// Redux Code checking for plugin, whether redux is installled in plugin too
			...
			public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
				...
				if ( empty ( $this->args['footer_credit'] ) ) {
					$this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'primestudio' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'primestudio' ) . '</a> v' . self::$_version ) . '</span>';
				}

				if ( empty ( $this->args['menu_title'] ) ) {
					$this->args['menu_title'] = esc_html__( 'Options', 'primestudio' );
				}

				if ( empty ( $this->args['page_title'] ) ) {
					$this->args['page_title'] = esc_html__( 'Options', 'primestudio' );
				}
				...
				if ( ! empty ( $this->args['opt_name'] ) ) {
					...
					{
						require_once trailingslashit( get_template_directory() ).'admin/redux-framework/core/dashboard.php';

						new reduxDashboardWidget($this);

						if ( ! isset ( $GLOBALS['redux_notice_check'] ) ) {
							require_once trailingslashit( get_template_directory() ).'admin/redux-framework/core/newsflash.php';
...
// get_wordpress_data()
			...
			public function show( $opt_name, $default = '' ) {
				...
				if ( ! is_array( $option ) && $option != '' ) {
					...
					printf('%s', $this->_get_default( $opt_name, $default ) );
				}
			}
// show()
...
// _admin_bar_menu()
			...
			public function _output_css() {
				...
				if($this->outputCSS !='')
				{
					/***** ADDED BY TRISTUP TO AVOID SOCCER STYLE ISSUE ****/
	wp_enqueue_style('primestudio-theme-options', get_template_directory_uri() . '/css/theme-options.css',['primestudio-style','primestudio-main']);
					
					wp_add_inline_style( 'primestudio-theme-options', $this->outputCSS );
				}
			}
					...
					if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
						...
						<script>
							...
							(function() {
								...
								wf.src = <?php echo ReduxFramework::$_url; ?>+'assets/js/webfont.js';
...
// _enqueue_output()
			...
			public function _enqueue() {
				require_once trailingslashit( get_template_directory() ).'admin/redux-framework/core/enqueue.php';
...
// _enqueue()
			...
			public function _load_page() {
				...
				if ( true == $this->show_hints ) {
					...
					$dismiss = 'true';
					$s       = esc_html__( 'Enable', 'primestudio' );

					// Values for disabling hints.
					if ( 'true' == $hint_status ) {
						$dismiss = 'false';
						$s       = esc_html__( 'Disable', 'primestudio' );
					}

					// Make URL
					$url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

					$event = esc_html__( 'moving the mouse over', 'primestudio' );
					if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
						$event = esc_html__( 'clicking', 'primestudio' );
					}

					// Construct message
					$msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'primestudio' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

					// Construct hint tab
					$tab = array(
						'id'      => 'redux-hint-tab',
						'title'   => esc_html__( 'Hints', 'primestudio' ),
						'content' => '<p>' . $msg . '</p>'
					);
...
// admin_footer_text()
			...
			private function get_default_output_string( $field ) {
				...
				if ( ! empty ( $default_output ) ) {
					$default_output = esc_html__( 'Default', 'primestudio' ) . ": " . substr( $default_output, 0, - 2 );
				}
			...
			public function ajax_save() {
				if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
					echo json_encode( array(
						'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'primestudio' ),
						'action' => ''
					) );
				...
				if ( ! current_user_can( $this->args['page_permissions'] ) ) {
					echo json_encode( array(
						'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'primestudio' ),
						'action' => ''
					) );
				...
				if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
					...
					if ( ! empty ( $values ) ) {
						...
						echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'primestudio' ) ) );
					}
				}
				if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
				...
				if ( isset( $return_array ) ) {
					if ( $return_array['status'] == "success" ) {
						require_once trailingslashit( get_template_directory() ).'admin/redux-framework/core/panel.php';
