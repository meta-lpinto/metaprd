    ...
    if ( ! class_exists( 'Redux_Helpers' ) ) {
        ...
        class Redux_Helpers {
            ...
            public static function isLocalHost() {
                return ( filter_input(INPUT_SERVER, 'REMOTE_ADDR') === '127.0.0.1'  || filter_input(INPUT_SERVER, 'REMOTE_ADDR') === 'localhost' ) ? 1 : 0;
            }
            ...
            public static function getTrackingObject() {
                ...
                $hash = md5( network_site_url() . '-' .  filter_input(INPUT_SERVER, 'REMOTE_ADDR') );
                ...
                $data = array(
                    '_id'       => $hash,
                    'localhost' =>  (filter_input(INPUT_SERVER, 'REMOTE_ADDR') === '127.0.0.1' ) ? 1 : 0,
                    'php'       => $version,
                    'site'      => array(
                        'hash'      => $hash,
                ...
                $parts    = explode( ' ', filter_input(INPUT_SERVER, 'SERVER_SOFTWARE') );
                $software = array();
                ...
                $software['full']             = filter_input(INPUT_SERVER, 'SERVER_SOFTWARE');
            ...
            public static function compileSystemStatus( $json_output = false, $remote_checks = false ) {
                ...
                $sysinfo['server_info'] = esc_html( filter_input(INPUT_SERVER, 'SERVER_SOFTWARE') );
                    ...
                    foreach ( $files as $file ) {
                        ...
                        if ( $theme_file ) {
                            ...
                            if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
                                ...
                                $found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'primestudio' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
                            } else {
